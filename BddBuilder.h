...
private:
	...
	void		__buildWireList(Wire * pwireHead,char** input_sequence,int& input_number);
	void		__buildWireListB(BddBuilder * pA, Wire * pwireHead);
	void		__buildGateList(Node * pNodeHead);
	void		__linkWires();
public:
	void		construct(Wire * pWireHead, Node * pNodeHead,char** input_sequence,int& input_number);	// build BDD
	void		constructB(BddBuilder * pA, Wire * pWireHead, Node * pNodeHead);	// build BDDB
	void		dotDump();					// DOT dump
	void		dotDumpC(DdNode ** ddNodes,int& number_of_diff_output,char** diff_output);// DOT dump(C)
	void		dotDumpCZdd(DdNode ** ddNodes);
	BddBuilder();
